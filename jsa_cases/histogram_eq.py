...
def get_cfd(image):
	...
	hist, bins = np.histogram(
		...
		bins=256,  # количество точек на x (отенков серого)
...
def equalize(img, cdf=None, use_cv_equ=False):
	...
	if use_cv_equ:
		# Используем бблиотеку
		return cv.equalizeHist(img)

	# Либо выравниваем цветность сами:
	# скрываем нули. нули у нас только для цветов темнее самого темного пкселя, имеющегося на изображении?
	...
	return cdf[img]  # не понимаю, как тут проходит преобразоване
